...
device = torch.device(
    f"cuda:{num_cuda_devices-1}" if torch.cuda.is_available() else "cpu"
)
...
def test_LoFTREncoderLayer():
    ...
    y = loftr_encoder_layer(x, source, None, None)
    assert y.shape == (B, L, n_embd)
...
def test_LoFTREncoderLayerPreLN():
    ...
    y = loftr_encoder_layer_post_ln(
        x, source, x_mask.to(device), source_mask.to(device)
    )
    assert y.shape == (B, L, n_embd)

    y = loftr_encoder_layer_post_ln(x, source, None, None)
    assert y.shape == (B, L, n_embd)
...
def test_LocalFeatureTransformer_coarse():
    ...
    feat0, feat1 = local_feature_transformer(
        x, source, x_mask.to(device), source_mask.to(device)
    )
    assert feat0.shape == (B, L, n_embd)
    assert feat1.shape == (B, S, n_embd)

    feat0, feat1 = local_feature_transformer(x, source, None, None)
    assert feat0.shape == (B, L, n_embd)
    assert feat1.shape == (B, S, n_embd)
